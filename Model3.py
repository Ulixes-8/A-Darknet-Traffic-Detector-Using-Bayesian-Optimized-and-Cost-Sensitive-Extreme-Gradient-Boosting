#Model 3:Benign Application Categorization with F1 Bayesian Optimization 
...
#Remove all Darknet rows from the dataset
...
param_space = {
    ...
    'max_depth': hp.quniform('max_depth', 20, 30, 1),
...
fig = px.parallel_coordinates(
    ...
    range_color=[0.7758999566503127,0.7758999566503127],
)
...
fig = px.parallel_coordinates(
    ...
    color_continuous_scale=px.colors.sequential.Turbo,
    range_color=[0.805949226892157,0.805949226892157],
)
...
for i, (ax, score_name, score_value) in enumerate(zip(axs, score_names, score_values)):
    ...
    ax.set_ylim([0.7, 1])  # Set y-axis limits to [0, 1]
...
# Collect the WEIGHTED scores for each fold
accuracy_scores = []
precision_scores = []
recall_scores = []
f1_scores = []
...
#Get the MACRO performance metrics
...
#Get the WEIGHTED performance metrics
...
#In[18] Produce a bar chart of the model performance (weighted and macro) on the test set
