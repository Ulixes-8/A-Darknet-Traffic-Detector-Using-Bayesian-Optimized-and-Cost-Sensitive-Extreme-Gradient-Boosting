...
#     'max_bin' : hp.quniform('max_bin', 2, 256, 1),
...
# # # perform grid search over the parameter grid using the validation set
# # best_average_score = 0
# # best_params = None
# # for learning_rate in param_grid['learning_rate']:
# #     for max_depth in param_grid['max_depth']:
# #         for n_estimators in param_grid['n_estimators']:
# #                 for max_bin in param_grid['max_bin']:
# #                     # create an instance of the XGBClassifier model with the current hyperparameters
# #                     xgb = XGBClassifier(random_state=0, gpu_id=0, tree_method='gpu_hist', predictor='gpu_predictor', objective='multi:softmax', eval_metric='merror',
# #                                         max_bin=max_bin, n_estimators=n_estimators, max_depth=max_depth, learning_rate=learning_rate)

# #                     # fit the XGBClassifier model on the training set
# #                     xgb.fit(X_train, y_train)

# #                     # evaluate the XGBClassifier model on the validation set (macro)
...
# #                     average_score = (recall_val + accuracy_val + precision_val + f1_val) / 4
                    
                    
# #                     # update the best hyperparameters if we have a better recall score
# #                     if average_score > best_average_score:
# #                         print(f"\n\nNEW BEST average score: {average_score:.4f} with parameters: {learning_rate, max_depth, n_estimators, max_bin}")
# #                         print(f"recall: {recall_val:.4f} with parameters: {learning_rate, max_depth, n_estimators, max_bin}")
# #                         print(f"accuracy: {accuracy_val:.4f} with parameters: {learning_rate, max_depth, n_estimators, max_bin}")
# #                         print(f"precision: {precision_val:.4f} with parameters: {learning_rate, max_depth, n_estimators, max_bin}")
# #                         print(f"f1: {f1_val:.4f} with parameters: {learning_rate, max_depth, n_estimators, max_bin}")
# #                         best_average_score = average_score
# #                         best_params = {'learning_rate': learning_rate, 'max_depth': max_depth, 'n_estimators': n_estimators, 'max_bin': max_bin}

# # # print the best hyperparameters and the recall score on the test set
...
# Classes
title = "XGBoost Confusion Matrix (Application Categorization)"
