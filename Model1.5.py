#Model 1.5: Cost Insensitive Darknet Detection with F1 Bayesian Optimization 
...
#Differences from Model 1 (which was cost sensitive):
#class_weight = None in Model 1.5, class_weight = 'balanced' in Model 1.
#scoring = F1 in Model 1.5, scoring = F2 in Model 1.
...
param_space = {
    ...
    'max_depth': hp.quniform('max_depth', 20, 25, 1),
...
def objective1(params):
    ...
    f1_scorer = make_scorer(f1_score, pos_label='Darknet', zero_division=1)
...
fig = px.parallel_coordinates(
    ...
    range_color=[0.944170898834894, 0.9474706317262219],
)
...
#Model 1 had class weight = 'Balanced', Model 1.5 has class weight = None
#This is what makes one cost sensitive and the other not cost sensitive
...
param_space = {
    ...
    'max_bin' : hp.quniform('max_bin', 768, 1600, 1),
...
fig = px.parallel_coordinates(
    search_results_df,
    dimensions=['n_estimators', 'max_depth', 'learning_rate', 'max_bin', 'scale_pos_weight', 'num_features', 'f1'],
    color='f1',
    labels={
        'n_estimators': 'Number of Estimators',
        'max_depth': 'Max Depth',
    ...
    color_continuous_scale=px.colors.sequential.Turbo,
    range_color=[0.9130431504476334,0.9566006373307051],
)
...
title = "XGBoost Confusion Matrix (Cost Insensitive Detection)"
...
#In[22] Explain the weights of the XGBoost model
